...
Of course, this cost analysis is fairly elementary, but the critical aspect of doing this in \dlambdaamor is that checking the proper terms (found in Appendix~\textbf{??}) against these types requires solving only linear constraints over $\R^+$, despite the cost bound being quadratic. Moreover, the terms themselves are reached by inserting the \texttt{ret}s, \texttt{bind}s, and \texttt{shift}s into the proper locations in the term one would already expect to have to write when writing this function.
