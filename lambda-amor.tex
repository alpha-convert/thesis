...
\subsubsection{Cost-Parametric Fold}
While many existing languages and type systems for (amortized) resource analysis also support higher-order functions, the allowable analyses with higher-order functions are limited. One such limitation is that function arguments to higher-order functions are usually assumed to be constant-cost: for instance, in the cost analysis of a fold, each application of the accumulation function is assumed to incur the same amount of cost.
