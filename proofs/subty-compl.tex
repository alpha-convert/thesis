\jtheorem{Proof of \autoref{thm:subty-compl}}{
  ...
  \jcase{12}{S-Impl}{
    \jgivengoal{
      ...
      \caseFact{2} $\Psi ; \Theta ; \Delta,\Phi_2 \pvdash \tau_1 \subty \tau_2 : \star$
     ...
     \caseFact{5} $\Theta ; \Delta, \Phi_2 \vDash \Phi$
     
     \caseText{By (3) and (5)}
     
     \caseFact{6} $\Theta ; \Delta \vDash (\Phi_2 \to \Phi) \wedge (\Phi_2 \to \Phi_1)$
     
     \caseText{Inverting (4)}
     
     \caseFact{7} $\Psi ; \Theta ; \Delta,\Phi_2 \vdash \texttt{eval}(\tau_1) \subtynf \texttt{eval}(\tau_2) : \star \gens \Phi$
     
     \caseText{By AS-Impl on (7)}
     
     \caseFact{8} $\Psi ; \Theta ; \Delta \vdash \Phi_1 \implies \texttt{eval}(\tau_1) \subtynf \Phi_2 \implies \texttt{eval}(\tau_2) : \star \gens (\Phi_2 \to \Phi) \wedge (\Phi_2 \to \Phi_1)$
